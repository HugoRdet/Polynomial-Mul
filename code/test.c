...
void test_algos(u_int64_t d,u_int64_t pas,int nb_essais){
	...
	for (u_int64_t i=pas;i<d;i+=pas){
		...
		for (u_int32_t n=0;n<nb_essais;n++){
			...
			tmp_start = clock ();
			mult_TC3(tmp_a,i,tmp_b,i, &res_TC3_tab, &taille_res);
			tmp_final = clock ();
			res_TC3+=((double)(tmp_final - tmp_start)/CLOCKS_PER_SEC);
			...
			for (u_int64_t j=0;j<i*2;j++){
				...
				if (res_naif_tab[j]!=res_TC3_tab[j]){
		...
		res_TC4/=nb_essais;
		res_TC3/=nb_essais;
		printf("%lld naif:%lf\tKaratsuba:%lf\tTC3:%lf\tTC4:%lf\n",i,res_naif,res_karat,res_TC3,res_TC4);
...
void test_algos_TCK(u_int64_t d,int pas,int nb_essais){
	...
	for (u_int64_t i=pas;i<d;i+=pas){
		...
		for (u_int32_t n=0;n<nb_essais;n++){
			...
			tmp_start = clock ();
			mult_TC3(tmp_a,i,tmp_b,i, &res_karat_tab, &taille_res);
			tmp_final = clock ();
			res_karat+=((double)(tmp_final - tmp_start)/CLOCKS_PER_SEC);
...
void test_algo(u_int64_t d,u_int64_t pas,int nb_essais){
	FILE *f=fopen("tests/test_TC","w");
	srand(time(NULL));
	for (u_int64_t i=pas+2;i<d;i+=pas){
		double res_karat=0.0;
